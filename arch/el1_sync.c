...
static void vpsci_handler(vcpu_t *vcpu)
{
	 ...
	 */
	u64 ret = vpsci_trap_smc(vcpu, vcpu->regs.x[0], vcpu->regs.x[1],
				 vcpu->regs.x[2]);
	vcpu->regs.x[0] = ret;
}
...
void el1_sync_proc()
{
	...
	case 0x16:
		LOG_NOTICE("[%s] hvc trap from EL1\n", __func__);
	...
	case 0x17:
		LOG_NOTICE("[%s] smc trap from EL1\n", __func__);
		...
		break;
	default:
		abort("Unknown el1 sync: esr_ec %p, esr_iss %p, elr %p, far %p",
		      esr_ec, esr_iss, elr, far);
		break;
	}
