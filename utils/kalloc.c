...
void kalloc_init(void)
{
	...
	for (u64 page = (u64)HIMAGE_END; page < PHYEND; page += PAGE_SIZE) {
		free_one_page((void *)page);
	}

	LOG_INFO("Kalloc have been initialized: \n");
	LOG_INFO("Kalloc: page start addr : 0x%x\n", (u64)HIMAGE_END);
	LOG_INFO("Kalloc: page numbers    : %d\n",
		 (PHYEND - (u64)HIMAGE_END) / PAGE_SIZE);
}
